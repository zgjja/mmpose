    ...
    def test_transform(self):
        ...
        for source_index, target_index in mapping:
            if isinstance(source_index, tuple):
                ...
                self.assertEqual(results['keypoints_visible'].ndim, 2)
                self.assertEqual(results['keypoints_visible'].shape,
                                 results['keypoints_visible_weights'].shape)
                self.assertTrue(
